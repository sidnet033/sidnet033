-- Switchboard construction standard (ArTuK / IEC 61439 / IEC 60439), a
-- fixed 3-option dropdown on the Project Detail switchboard table.
alter table switchboards
  add column std text check (std in ('ArTuK', '61439', '60439'));

-- carry std forward when cloning/revisioning, same as the other switchboard columns
...
begin
  ...
  insert into switchboards (
    ...
    switchboard_type_id, description, ip_rating, qty, std
  )
  values (
...
begin
  ...
  for v_sb in select * from switchboards where revision_id = p_revision_id order by sort_order loop
    ...
    values (
      ...
      v_sb.switchboard_type_id, v_sb.description, v_sb.ip_rating, v_sb.qty, v_sb.std
    )
    returning id into v_new_sb_id;
